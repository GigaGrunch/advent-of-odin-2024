...
runners := []struct{file_path: string, dimensions: Vec, expected_result: Maybe(Vec)} {
    { "day18_test.txt", Vec{7, 7}, Vec{6, 1} },
    { "day18_input.txt", Vec{71, 71}, nil },
}
...
execute :: proc(input: string, dimensions: Vec) -> Vec {
    ...
    for line in strings.split_lines_iterator(&line_it) {
        ...
        if !has_path(field_map, dimensions) do break
    }
    
    return coord
}

has_path :: proc(field_map: []u8, dimensions: Vec) -> bool {
    start_pos := Vec{0, 0}
...
main :: proc() {
    ...
    for runner in runners {
        ...
        result := execute(transmute(string)input, runner.dimensions)
