...
runners := []struct{file_path: string, swap_pairs: int, expected_result: Maybe(string)} {
...
Gate_Index :: distinct int

execute :: proc(input: string, swap_pairs: int) -> string {
    ...
    swap_gates := make([]Gate_Index, swap_pairs * 2)
    ...
    outer: for {
        ...
        if !has_dupes {
            fmt.printfln("swap: %v", swap_gates)
        
            pending_gates: [dynamic]Gate_Index
            defer delete(pending_gates)
            for i in 0..<len(all_gates) do append(&pending_gates, Gate_Index(i))
            ...
            for len(pending_gates) > 0 {
                for pending_gate_i_i := len(pending_gates) - 1; pending_gate_i_i >= 0; pending_gate_i_i -= 1 {
                    pending_gate_i := pending_gates[pending_gate_i_i]
                    gate := all_gates[pending_gate_i]
                    for swap_gate_i_i := 0; swap_gate_i_i < len(swap_gates); swap_gate_i_i += 2 {
                        swap_gate_i := swap_gates[swap_gate_i_i]
                        swap_partner_i := swap_gates[swap_gate_i_i + 1]
                        if pending_gate_i == swap_gate_i do gate.out = all_gates[swap_partner_i].out
                        else if pending_gate_i == swap_partner_i do gate.out = all_gates[swap_gate_i].out
                    }
                    
                    if gate.lhs not_in values || gate.rhs not_in values do continue
                    assert(gate.out not_in values)
                    defer unordered_remove(&pending_gates, pending_gate_i_i)
                    switch gate.op {
            ...
            fmt.printfln("%v + %v = %v", x, y, z)
            if x + y == z do break outer
        }
        
        for i in 0..<len(swap_gates) {
            swap_gates[i] = (swap_gates[i] + 1) % Gate_Index(len(all_gates))
...
main :: proc() {
    ...
    for runner in runners {
        ...
        result := execute(transmute(string)input, runner.swap_pairs)
        defer delete(result)
