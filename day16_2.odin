...
execute :: proc(input: string) -> int {
    ...
    cost_frontier: [dynamic]Transform
    defer delete(cost_frontier)
    
    append(&cost_frontier, Transform {
        pos = start_pos,
        dir = .right,
    })
    ...
    for len(cost_frontier) > 0 {
        current := pop(&cost_frontier)
        ...
        for candidate in candidates {
            ...
            append(&cost_frontier, candidate)
        }
    }
    
    min_target_cost := max(int)
    for cost in plan_at(min_cost, end_pos) do min_target_cost = min(min_target_cost, cost)
    
    best_path_tiles: map[Vec]struct{}
    defer delete(best_path_tiles)
    
    path_frontier: [dynamic]Transform
    defer delete(path_frontier)
    
    for cost, dir in plan_at(min_cost, end_pos) {
        if cost == min_target_cost {
            best_path_tiles[end_pos] = {}
            append(&path_frontier, Transform {
                pos = end_pos,
                dir = dir,
            })
        }
    ...
    for len(path_frontier) > 0 {
        ...
        for candidate in candidates {
            ...
            if plan_at(plan, candidate.pos) == '#' do continue
            candidate_cost := current_cost - (1 if candidate.dir == current.dir else 1000)
            if plan_at(min_cost, candidate.pos)[candidate.dir] != candidate_cost do continue
