...
execute :: proc(input: string) -> int {
    ...
    print_map(field_map[:], width, height)
    fmt.printfln("width: %v, height: %v, start: %v, end: %v", width, height, start_pos, end_pos)
    
    path_length := shortest_path_length(field_map[:], width, height, start_pos, end_pos)
...
shortest_path_length :: proc(field_map: []u8, width, height: int, start_pos, end_pos: Vec) -> int {
    ...
    for len(frontier) > 0 {
        ...
        for frontier_pos in frontier {
            if get_direct_distance(frontier_pos, end_pos) < get_direct_distance(pos, end_pos) {
                pos = frontier_pos
            }
        }
        ...
        for direction in DIRECTIONS {
            ...
            tentative_path_length := neighbor_path_length^ + 1
